...
import com.example.gnssandopticalflowapp.function.gnss.gnss_source.celes_trak.CelesTrakSatelliteRepository
import com.example.gnssandopticalflowapp.function.gnss.gnss_source.pvt.GnssSatellitePVTResolver
import com.example.gnssandopticalflowapp.function.gnss.gnss_source.approximation.SatelliteCalculator
import com.example.gnssandopticalflowapp.function.gnss.gnss_source.celes_trak.Sgp4OrbitPropagator
import com.example.gnssandopticalflowapp.function.gnss.gnss_source.igs.IgsBroadcastEphemerisPropagator
import com.example.gnssandopticalflowapp.function.gnss.gnss_source.igs.IgsBroadcastEphemerisRepository
...
class GnssSatelliteTracker {
    ...
    private fun selectBroadcastEphemeris(
        ...
        return latestBroadcastEphemerides[satelliteKey]
            ?.minByOrNull { record -> abs(observationUtcMillis - record.epochUtcMillis) }
            ?.takeIf { record ->
                abs(observationUtcMillis - record.epochUtcMillis) <= BROADCAST_MAX_AGE_MS
            }
    }
